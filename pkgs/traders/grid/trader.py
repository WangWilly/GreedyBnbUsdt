...
class TraderGrid:
    def __init__(self, config: TraderGridConfig, exchange: ExchangeClient):
        ...
        self.risk_manager = ManagerAdvancedRisk(risk_manager_cfg, self, self.exchange)
        ...
        self.actioner_s1 = ActionerS1(actioner_s1_cfg, self, self.risk_manager, self.exchange)
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.symbol_info = self.exchange.exchange.market(self.cfg.SYMBOL)
            ...
            if self.base_price is None:
                raise ValueError("Failed to fetch initial base price, please check the market data.")
            ...
            try:
                ...
                latest_trades = await self.exchange.fetch_my_trades(self.cfg.SYMBOL, limit=10)
                if latest_trades:
    ...
    async def _ensure_trading_funds(self):
        ...
        try:
            ...
            # 计算所需资金
            required_usdt = self.cfg.MIN_TRADE_AMOUNT * 2  # 保持两倍最小交易额
    ...
    async def execute_order(self, side):
        ...
        while retry_count < max_retries:
            try:
                # 获取最新订单簿数据
                order_book = await self.exchange.fetch_order_book(self.cfg.SYMBOL, limit=5)
                ...
                # 检查订单状态
                updated_order = await self.exchange.fetch_order(order_id, self.cfg.SYMBOL)
                ...
                try:
                    ...
                    try:
                        check_order = await self.exchange.fetch_order(order_id, self.cfg.SYMBOL)
...
"""
    ...
    async def _adjust_grid_after_trade(self):
        """根据市场波动动态调整网格大小"""
        trade_count = self.order_manager.trade_count
        if trade_count % self.cfg.GRID_PARAMS['adjust_interval'] == 0:
            volatility = await self._calculate_volatility()
            
            # 根据波动率调整
            if volatility > self.cfg.GRID_PARAMS['volatility_threshold']['high']:
                new_size = min(
                    self.grid_size * 1.1,  # 扩大10%
                    self.cfg.GRID_PARAMS['max']
                )
                action = "扩大"
            else:
                new_size = max(
                    self.grid_size * 0.9,  # 缩小10%
                    self.cfg.GRID_PARAMS['min']
                )
                action = "缩小"
    ...
    async def _reinitialize(self):
        ...
        try:
            ...
            self.highest = None
            self.lowest = None
            self.grid_size = self.cfg.GRID_PARAMS['initial']
            self.last_trade = 0
        ...
        for order_id, timestamp in list(self.order_timestamps.items()):
            if current_time - timestamp > self.ORDER_TIMEOUT:
                try:
                    ...
                    order = await self.exchange.fetch_order(order_id, self.cfg.SYMBOL, params)
                    ...
                    if order['status'] == 'closed':
                        ...
                        await self.exchange.cancel_order(order_id, self.cfg.SYMBOL, params)
    ...
    async def calculate_trade_amount(self, side, order_price):
        ...
        risk_adjusted_amount = min(
            total_assets * self.cfg.RISK_FACTOR * volatility_factor * kelly_f * percentile_factor,
            total_assets * self.cfg.MAX_POSITION_RATIO
        )
        
        # 应用最小/最大限制
        amount_usdt = max(
            min(risk_adjusted_amount, self.cfg.BASE_AMOUNT),
    ...
    async def _get_order_price(self, side):
        """获取订单价格"""
        try:
            order_book = await self.exchange.fetch_order_book(self.cfg.SYMBOL)
    ...
    async def _get_price_percentile(self, period='7d'):
        ...
        try:
            # 获取过去7天价格数据（使用4小时K线）
            ohlcv = await self.exchange.fetch_ohlcv(self.cfg.SYMBOL, '4h', limit=42)  # 42根4小时K线 ≈ 7天
    ...
    async def _calculate_required_funds(self, side):
        ...
        return min(required, self.cfg.MAX_POSITION_RATIO * total_assets)
    ...
    async def _check_flip_signal(self):
        ...
        try:
            ...
            flip_threshold = self.base_price * TraderGridConfig.flip_threshold(self.grid_size)
    ...
    def _calculate_dynamic_base(self, total_assets):
        ...
        # 计算基于总资产百分比的交易金额范围
        min_amount = max(
            self.cfg.MIN_TRADE_AMOUNT,  # 不低于20 USDT
            total_assets * self.cfg.MIN_POSITION_PERCENT  # 不低于总资产的5%
        )
        max_amount = total_assets * self.cfg.MAX_POSITION_PERCENT  # 不超过总资产的15%
    ...
    async def get_available_balance(self, currency):
        ...
        return balance.get('free', {}).get(currency, 0) * self.cfg.SAFETY_MARGIN
