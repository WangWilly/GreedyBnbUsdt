...
class TraderGrid:
    ...
    async def _check_and_transfer_initial_funds(self):
        ...
        try:
            # 获取现货和理财账户余额
            balance = await self.exchange.fetch_balance() # Keep for direct spot balance access
            funding_balance = await self.exchange.fetch_funding_balance() # Keep for direct funding balance access
    ...
    async def _calculate_order_amount(self):
        ...
        try:
            ...
            if hasattr(self, cache_key) and current_time - getattr(self, f'{cache_key}_time') < 60:  # 1分钟缓存
    ...
    async def execute_order(self, side):
        ...
        while retry_count < max_retries:
            try:
                ...
                amount = self.position_manager.adjust_amount_precision(amount_usdt / order_price)
                
                # 检查余额是否足够
                # ensure_trading_funds can be called here with specific amounts
                if not await self.position_manager.ensure_trading_funds(side, amount_usdt):
                    self.logger.warning(f"{side.capitalize()}余额不足或划转失败，第 {retry_count + 1} 次尝试中止")
                    return False
    ...
    async def check_buy_balance(self):
        ...
        try:
            ...
            if await self.position_manager.ensure_trading_funds('BUY', amount_usdt):
                self.logger.info(f"买入资金已确认或准备就绪: {amount_usdt:.2f} {self.position_manager.quote_currency}")
                return True
            else:
                self.logger.error(f"买入资金不足或准备失败: {amount_usdt:.2f} {self.position_manager.quote_currency}")
                return False
    ...
    async def check_sell_balance(self):
        ...
        try:
            ...
            if await self.position_manager.ensure_trading_funds('SELL', amount_usdt):
                self.logger.info(f"卖出资金已确认或准备就绪 (等值 {amount_usdt:.2f} {self.position_manager.quote_currency})")
                return True
            else:
                self.logger.error(f"卖出资金不足或准备失败 (等值 {amount_usdt:.2f} {self.position_manager.quote_currency})")
