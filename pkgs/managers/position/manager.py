...
class ManagerPosition:
        ...
        try:
            ...
            while not self.exchange.markets_loaded and retry_count < 3:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to load markets, retrying... ({retry_count + 1}/3)\n{str(e)}"
                    )
                    retry_count += 1
                    if retry_count >= 3:
                        raise
    ...
    async def get_available_balance(self, currency: str) -> float:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get available balance for {currency}: {str(e)}"
            )
            return 0
    ...
    async def get_total_assets(self) -> float:
        ...
        try:
            ...
            default_total = (
                self._assets_cache["value"] if self._assets_cache["value"] else 0
            )
            ...
            if not balance:
                self.logger.error(
                    "Failed to get balance, returning default total assets"
                )
                return default_total
            ...
            spot_quote += float(
                balance.get("used", {}).get(self.quote_currency, 0) or 0
            )
            ...
            # Update cache
            self._assets_cache = {"time": current_time, "value": total_assets}

            # Only log when asset value changes significantly
            if (
                not self._last_logged_assets
                or abs(total_assets - self._last_logged_assets)
                / max(self._last_logged_assets, 0.01)
                > 0.01
            ):
                self.logger.info(
    ...
    async def get_position_value(self) -> float:
        ...
        try:
            ...
            base_amount = float(
                balance.get("free", {}).get(self.base_currency, 0) or 0
    ...
    async def transfer_to_spot(self, currency: str, amount: float) -> bool:
        ...
        try:
            self.logger.info(
                f"Transferring {amount:.8f} {currency} from savings to spot"
            )
            await self.exchange.transfer_to_spot(currency, amount)
            return True
        except Exception as e:
            self.logger.error(
                f"Failed to transfer {amount} {currency} to spot: {str(e)}"
            )
            return False
    ...
    async def transfer_to_savings(self, currency: str, amount: float) -> bool:
        ...
        try:
            self.logger.info(
                f"Transferring {amount:.8f} {currency} from spot to savings"
            )
            await self.exchange.transfer_to_savings(currency, amount)
            return True
        except Exception as e:
            self.logger.error(
                f"Failed to transfer {amount} {currency} to savings: {str(e)}"
            )
            return False
        ...
        try:
            ...
            if side.upper() == "BUY":
                ...
                if usdt_available + funding_usdt < usdt_needed:
                    self.logger.error(
                        f"Insufficient {self.quote_currency} for buy order: needed {usdt_needed:.2f}, available {usdt_available:.2f}, savings {funding_usdt:.2f}"
                    )
                    return False
                ...
                if base_available + funding_base < base_needed:
                    self.logger.error(
                        f"Insufficient {self.base_currency} for sell order: needed {base_needed:.8f}, available {base_available:.8f}, savings {funding_base:.8f}"
                    )
                    return False
    ...
    async def transfer_excess_funds(self) -> bool:
        ...
        try:
            ...
            if not current_price or current_price <= 0 or total_assets <= 0:
                self.logger.warning(
                    "Invalid price or total assets, skipping excess funds transfer"
                )
                return False
            ...
            spot_quote_balance = float(
                balance.get("free", {}).get(self.quote_currency, 0) or 0
            )
            ...
            if spot_quote_balance > target_quote_hold:
                ...
                if transfer_amount > 1.0:
                    self.logger.info(
                        f"Transferring excess {self.quote_currency} to savings: {transfer_amount:.2f}"
                    )
                    try:
                        await self.exchange.transfer_to_savings(
                            self.quote_currency, transfer_amount
                        )
                        transfer_executed = True
                    except Exception as e:
                        self.logger.error(
                            f"Failed to transfer {self.quote_currency} to savings: {str(e)}"
                        )
                else:
                    self.logger.info(
                        f"Excess {self.quote_currency} ({transfer_amount:.2f}) too small, not transferring"
                    )
            ...
            if spot_base_balance > target_base_hold_amount:
                ...
                if transfer_amount >= 0.01:
                    self.logger.info(
                        f"Transferring excess {self.base_currency} to savings: {transfer_amount:.4f}"
                    )
                    try:
                        await self.exchange.transfer_to_savings(
                            self.base_currency, transfer_amount
                        )
                        transfer_executed = True
                    except Exception as e:
                        self.logger.error(
                            f"Failed to transfer {self.base_currency} to savings: {str(e)}"
                        )
                else:
                    self.logger.info(
                        f"Excess {self.base_currency} ({transfer_amount:.4f}) too small, not transferring"
                    )
